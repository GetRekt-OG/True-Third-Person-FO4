...
namespace TrueThirdPerson {
    constexpr auto kTargetRuntime = REL::CreateRuntime(1, 11, 240, 0);
    ...
    constexpr std::size_t kHandleLookInputIndex = 0x15;
    ...
    struct ThirdPersonCameraOrbitHook {
        ...
        static bool CameraContextAvailable() {
            ...
            const auto *ui = RE::UI::GetSingleton();
            return g_gameReady && ui && (ui->menuMode == 0 || ConsoleOpen());
        }
        ...
        static void RefreshHipFire(RE::PlayerCharacter *player) {
            ...
            if (player->gunState == RE::GUN_STATE::kFire)
                g_hipFireUntil = EquipGuard::Now() + 0.275;
        ...
        static void HipFireButton(const RE::ButtonEvent *button) {
            if (!button->QJustPressed() ||
                std::string_view(button->QUserEvent().c_str()) != "Attack")
                return;
            auto *player = RE::PlayerCharacter::GetSingleton();
            if (!HipFireContext(player))
                return;
        ...
        static void RebaseLockRelease(void *raw) {
            if (!g_lockReleaseFrames)
                return;
            auto *player = RE::PlayerCharacter::GetSingleton();
            if (!player || !MeleeLock::ProtectCamera() || MeleeLock::lockRequested) {
        ...
        static void HandleLookInputThunk(void *a_self, const Point2 &a_input) {
            if (!GameWindowFocused() || ConsoleOpen())
                return;
            if (MeleeLock::Active())
                return;
            SyncAimReleaseBeforeCameraUpdate();
            if (!Active(a_self)) {
                g_savedFreeRotationValid = false;
                HandleLookInputFunc(a_self, a_input);
                return;
            }
    ...
    struct DirectionalHooks {
        static void Movement(RE::PlayerCharacter *self, float dt, RE::NiPoint3 &move,
            ...
            if (self != RE::PlayerCharacter::GetSingleton() || !IndependentLocomotionActive(self))
                return;
            auto *controls = RE::PlayerControls::GetSingleton();
            if (!controls)
                return;
...
F4SE_PLUGIN_VERSION = []() consteval noexcept {
    ...
    info.SetPluginVersion(REX::Version(0, 2, 4, 17));
    info.SetUseAddressLibrary_RuntimeAE(true);
    info.SetIsLayoutDependent_RuntimeAE(true);
    info.SetCompatibleVersions(std::array{TrueThirdPerson::kTargetRuntime});
    return info;
}();
F4SE_PLUGIN_LOAD(const F4SE::LoadInterface *f4se) {
    TTPDiagnostic::Trace("=== TrueThirdPerson 0.2.4.17 lock-only mouse controls ===");
    if (!f4se || f4se->IsEditor() || f4se->GetRuntimeVersion() != REX::Version(1, 11, 240, 0)) {
